...
const SCORE_LEGEND = [
  ...
  { factor: 'Trend (52W)',     weight: 25,  detail: 'CSP curve: ≤5%=25 · 5–10%→25→18.3 · 10–20%→18.3→11.7 · 20–30%→11.7→0 · >30%=0.',
    definition: 'Direction-aware trend factor based on distance from the 52-week (252-trading-day) high. v3 collapsed v2 SMA Alignment (15) and 52W (10) into a single 25-pt direction-aware factor — both were measuring "is this uptrending?".',
    why: 'For CSPs: stocks near their highs have upward momentum and are less likely to gap down through your strike. Far below the 52W high signals a downtrend — puts sold there carry much higher assignment risk. Smooth continuous decay from 25 at ≤5% to 0 at 30%.',
    formula: 'dist = (Closeₜ − max(Close, 252d)) / max(Close, 252d) × 100\n  pct_below = abs(min(dist, 0))\n  v3: replaces SMA + 52W (combined 25 pts) with one direction-aware factor' },
  ...
  { factor: 'Delta',            weight: 20,  detail: 'Symmetric bell · |Δ−(−0.225)| ≤ 0.025 = 20 · ≤ 0.075 = 13 · ≤ 0.125 = 7 · outside gate = 0.',
    ...
    why: 'Delta approximates the probability of expiring in-the-money. −0.225 is the sweet spot for premium vs. risk. v3 audit fix #7: aggressive (Δ < −0.30) and conservative (Δ > −0.15) wings now score equally at the same offset from ideal — v2 favored the riskier wing 5.83 vs 5.0.',
    formula: 'Black-Scholes put delta:\n  d1 = (ln(S/K) + (r + 0.5σ²)T) / (σ√T)\n  delta = N(d1) − 1\n  v3: symmetric offset-based bell, ideal = −0.225\n  Gate (−0.35, −0.10) enforced upstream' },
  { factor: 'Bid-Ask Spread',  weight: 30,  detail: '≤1%=30 · 1–3%→30→20 · 3–5%→20→11 · 5–8%→11→3 · >8%=0.',
    ...
    why: 'Wide spreads directly erode realized premium. A 10% spread on a $1.00 put loses $0.05–$0.10 on entry alone, and you pay it again on every roll. v3 weighted execution quality up to 30 pts because it actually differentiates ranking within the gate.',
  ...
  { factor: 'Annualized ROC',   weight: 35,  detail: '≥20%=35 · 14–20%→24.5→35 · 8–14%→14→24.5 · 4–8%→3.5→14 · 2–4%→0→3.5 · <2%=0.',
    ...
    why: 'ROC is the actual yield — the primary objective for a premium seller. v3 elevated this to 35 pts (the largest weight) because two strikes with identical Δ and spread can have wildly different yields. Cliff fix #6 added a 2–4% ramp for continuous behavior.',
