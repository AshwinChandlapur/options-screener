    ...
    <div className="csp-backtest-panel" style={{
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #334155', textAlign: 'right', opacity: 0.7 }}>
          ...
          <tbody>
            {buckets.map(b => (
              <tr key={b.bucket} style={{ borderBottom: '1px solid #1e293b', textAlign: 'right', opacity: b.n === 0 ? 0.4 : 1 }}>
                <td style={{ textAlign: 'left', padding: '4px 8px' }}>{b.bucket}{b.bucket === '65-75' && ' (tradeable)'}</td>
                <td>{b.n}</td>
                <td style={{ color: b.n ? rocColor(b.mean_roc) : undefined }}>{b.n ? fmtPct(b.mean_roc) : '—'}</td>
                <td>{b.n ? fmtPct(b.median_roc) : '—'}</td>
                <td>{b.n ? b.win_rate.toFixed(0) + '%' : '—'}</td>
                <td>{b.n ? b.assign_rate.toFixed(0) + '%' : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
