...
def test_fetch_unclassified_query_targets_undefined_conviction_direction(
    ...
    assert "NOT IS_DEFINED(c.conviction_direction)" in call.kwargs["query"]
...
def test_fetch_missing_embeddings_query_targets_conviction_set_and_no_embedding(
    ...
    assert "IS_DEFINED(c.conviction_direction)" in query
    assert "NOT IS_DEFINED(c.embedding)" in query


def _axes(direction: str = "bull", substance: str = "researched") -> dict:
    return {
        "direction": direction,
        "substance": substance,
        "driver": "earnings",
...
def test_write_conviction_without_embedding_omits_embedding_fields(
    ...
    cosmos_client.write_conviction(
        {"id": "1", "ticker": "NVDA"}, _axes(), 0.9
    )

    payload = fake_container.upsert_item.call_args.args[0]
    assert payload["conviction_direction"] == "bull"
    assert payload["conviction_substance"] == "researched"
    assert payload["conviction_driver"] == "earnings"
    assert payload["conviction_position"] == "entering"
...
def test_write_conviction_with_embedding_requires_model(
    ...
    with pytest.raises(ValueError, match="embedding_model is required"):
        cosmos_client.write_conviction(
            {"id": "1"}, _axes(), 0.9, embedding=[0.1], embedding_model=None
        )
...
def test_write_embedding_upserts_doc_with_embedding_fields(
    ...
    cosmos_client.write_embedding(
        {"id": "1", "ticker": "NVDA", "conviction_direction": "bull"},
        [0.1, 0.2, 0.3],
