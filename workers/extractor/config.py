...
class ExtractorConfig:
    ...
    # How long (seconds) the EH receive thread runs. Default leaves headroom
    # for cold-start (KV fetches + AMQP handshake ≈ 8–15s). See ADR-0016.
    receive_window_seconds: int = 25
    # If True, start from the earliest available EH offset (full replay).
    # Default False = start from latest (skip backlog on fresh deploy).
