...
def decay_weighted_density(
    ...
    if weight_sum == 0.0:
        return 0.0
    # Divide by max possible weight (all weight at t=0) to normalize to [0,1].
    # Max = weight_sum if every day had count=1; we normalize by weight_sum
    # so the result reflects density relative to the window's own weight budget.
    return min(weighted_count / weight_sum, 1.0)
...
def build_snapshot(
    ...
    for sig in signals:
        ...
        sig_date = date.fromtimestamp(ts) if ts else bucket_date
